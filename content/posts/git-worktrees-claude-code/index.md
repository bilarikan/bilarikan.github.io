...
The other concrete outcome: knowing that Claude works in a worktree, the expected end-of-session step for any task that pushes changes is a `git pull` in the main working directory. It is not a failure or a sync problem -- it is just how the model works.
